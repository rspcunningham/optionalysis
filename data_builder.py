...
def getOption(symbol, toCSV=False, timeIt=False, bareMinimum=False, rawData=False):
    ...
    if bareMinimum:
        ...
        allOpt.drop(
            dropThese, axis=1, inplace=True
        )  # reduces from 137 kb to 80 kb in case of apple
    if not rawData:
        # remove any mini options (not regular), and bad puts and calls, etc.
        allOpt.drop(
            allOpt[(allOpt.type == 0) & (allOpt.strike <= price)].index, inplace=True
        )
        allOpt.drop(
            allOpt[(allOpt.type == 1) & (allOpt.strike >= price)].index, inplace=True
        )
...
def getListOptions(
    tickerList, toCSV=False, timeIt=False, bareMinimum=False, rawData=False
):
    start = time.time()
...
chain = getOption(
    "TSLA", toCSV=True, rawData=True
)  # averages between 6 and 8 seconds for a big stock
